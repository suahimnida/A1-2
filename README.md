...
**1차 추천 (Gemini)**: 입력 날짜 기준 여행하기 좋은 국내 도시와 날씨/행사 정보를 JSON으로 생성
<br>**맛집 검색 (카카오맵)**: 추천 도시 기준 맛집 최대 5곳 검색</br>
**최종 리포트 생성 (Gemini)**: 위 결과를 종합해 Markdown 여행 리포트 작성

각 단계는 실행 중 진행 로그(`[HH:MM:SS] ...`)로 상태를 보여주며, 완료되면 결과 파일이 저장된 경로를 안내합니다
<br>맛집 검색이 실패하거나 결과가 0건이어도 프로그램은 중단되지 않고 "데이터 없음"으로 표기한 채 리포트 생성까지 이어집니다</br>
...
- VS Code의 실행(F5) 버튼으로 돌릴 경우, `-date` 인자가 자동으로 전달되지 않아
  <br>`error: the following arguments are required: -date` 오류가 날 수 있습니다</br>
- 이때는 터미널에서 직접 실행하거나, `.vscode/launch.json`의 `"args"`에 `["-date", "2026-10-05"]`를 추가하세요
...
<br>그 경로를 복사해 파일 탐색기나 VS Code에서 바로 열어보면 됩니다</br>
